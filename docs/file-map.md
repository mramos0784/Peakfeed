...
| `/profile` | `(app)/profile/page.tsx` | required | Self-view: username/city/join date from `profiles`, follower/following counts (ADR 0011), `UserSearchBox`, recent votes from `votes` (full entry shape since ADR 0009, renders `EntryActionMenu` per row) |
| `/profile/[username]` | `(app)/profile/[username]/page.tsx` | required | Any other user's profile (ADR 0011) — thinner than self-view, no recent votes/log-out. Avatar, city, join date, follower/following counts, `FollowButton`. Own username redirects to `/profile` |
...
| `api/follows` | POST/DELETE `{followedId}` (ADR 0011), follow/unfollow — no approval step, a repeat follow upserts onto the same composite-key row rather than erroring |
| `api/users/search` | GET `?q=`, partial case-insensitive `profiles.username` match (ADR 0011) — the discovery path for follow/unfollow this session, no user directory exists |
...
| `InListSearchForm.tsx` | Client component: structured, named-field in-list search (ADR 0008) — distinct from `AddToListsButton`'s single free-text box. Category fixed by the list it's rendered in: Songs save directly (internal-key dedup, no candidates); Movies/Events/Creators/Issues/Restaurants/Venues (ADR 0012) search Wikidata + web in parallel and show a confirm step, including a verified source link when the search found one. Restaurants/Venues share one `isPlace` query field and fall back to the user's own `profiles.city` (`homeCity`) when a search result carries no subtitle |
...
| `FollowButton.tsx` | Client component (ADR 0011): Follow/Following toggle, POST/DELETE `/api/follows`, optimistic local state |
| `UserSearchBox.tsx` | Client component (ADR 0011): username search, links results to `/profile/[username]`. Temporary home on self-view `/profile` — moves to Feed once that screen exists |
...
| `parseLink.ts` | Share-ingestion resolution: pattern-matches Spotify/Google Maps URLs first, escalates to the Anthropic API server-side only when no clean pattern matches. Short-circuits known-unsupported sources (Amazon Music, share.google) and filters bot-block/interstitial titles before they'd otherwise be presented as a guess. Also exports `webSearchCandidates()` (multi-result search, distinct from the single-answer `webSearchExtractEvent`) and `sourceToProvenance()` (maps internal resolution `source` to the persisted `ResolutionProvenance` enum). Each `SearchCandidate` carries a `source_url` (ADR 0012), verified against the real URLs the web-search API actually returned before being trusted, same anti-hallucination check `webSearchExtractEvent`'s `sources` list already used |
